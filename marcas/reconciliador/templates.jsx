  ...
  function TplCloseCTA({ title, sub, ctaText, page = '6 / 6', dark = true }) {
    return (
      <div className={`t-post ${dark ? 't-post--dark' : ''}`}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <div className="t-h2" style={{ color: dark ? 'var(--papel)' : 'var(--navy)', fontSize: 84 }}>{title}</div>
          {sub && <div className="t-body" style={{ marginTop: 32, marginBottom: 56, fontSize: 32, maxWidth: '26ch' }}>{sub}</div>}
          <div className="t-cta">{ctaText} <span style={{ fontSize: 30 }}>→</span></div>
        </div>
        <PostFoot pages={page} />
      ...
      <div className="t-post">
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', left: -16, top: -40, fontFamily: 'var(--font-script)', fontSize: 180, lineHeight: 0.8, color: 'var(--gold)', opacity: 0.3 }}>“</div>
    ...
    return (
      <div className="t-post t-post--dark t-post--photo">
        ...
        <div className="t-photo-inner" style={{ padding: '104px 96px 108px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 116, lineHeight: 1, color: 'var(--papel)', textWrap: 'balance' }}>{date}</div>
          <div style={{ fontFamily: 'var(--font-wide)', fontSize: 36, fontWeight: 600, letterSpacing: '0.04em', color: 'var(--papel-3)', marginTop: 20 }}>{title}{title && time ? ' · ' : ''}{time}</div>
          {place && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 18, marginTop: 44, marginBottom: 'auto', color: 'var(--papel)' }}>
              ...
              <div style={{ fontFamily: 'var(--font-sans)', fontSize: 34, fontWeight: 500 }}>{place}</div>
            </div>
          )}
          {cta && (
            <div style={{ marginTop: 56 }}>
              <span style={{ display: 'inline-block', background: 'var(--papel)', color: 'var(--navy)', fontFamily: 'var(--font-wide)', fontSize: 26, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', padding: '22px 44px', borderRadius: 999 }}>{cta}</span>
            </div>
          )}
        </div>
      </div>
    ...
    return (
      <div className="t-post t-post--photo">
        ...
        <div className="t-photo-inner">
          ...
          <div style={{ flex: 1 }} />
          <div style={{ fontFamily: 'var(--font-script)', fontSize: 56, lineHeight: 1.25, color: 'var(--papel)', textWrap: 'balance', maxWidth: '20ch', marginBottom: 24 }}>"{quote}"</div>
          {who && <div style={{ fontFamily: 'var(--font-wide)', fontSize: 20, fontWeight: 600, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--gold-claro)' }}>— {who}</div>}
          <div style={{ height: 56 }} />
          <PostFoot pages="" />
        </div>
      </div>
      ...
      <div className="t-post t-post--dark">
        ...
        <div className="t-photo-inner" style={{ padding: '36px 96px 64px' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}>
            <div style={{ color: 'var(--papel)', fontSize: 28, fontWeight: 600, marginBottom: 16 }}>{intro}</div>
            <div className="t-eyebrow" style={{ color: 'var(--gold)', marginBottom: 8 }}>{kicker}</div>
            <div className="t-title" style={{ color: 'var(--papel)', fontSize: 84, lineHeight: 1.1 }}>{title} {titleEm && <em style={{ color: 'var(--gold-claro)' }}>{titleEm}</em>}</div>
            {body && <div className="t-body" style={{ marginTop: 32, color: 'var(--papel-3)', maxWidth: '32ch', fontSize: 28, lineHeight: 1.45 }}>{body}</div>}
            {cta && <div style={{ marginTop: 44, display: 'inline-flex', alignSelf: 'flex-start', padding: '18px 30px', background: 'var(--gold)', color: 'var(--navy)', fontFamily: 'var(--font-wide)', fontSize: 22, fontWeight: 700, letterSpacing: '0.04em', borderRadius: 8 }}>{cta}</div>}
          </div>
          <PostFoot pages="" />
        </div>
      </div>
      ...
      <div className="t-story t-lectionary" style={{ backgroundColor: 'var(--papel-2)' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 24 }}>
          ...
          <div style={{ marginTop: 18, textAlign: 'center' }}>
            <div style={{ fontFamily: 'var(--font-wide)', fontSize: 15, fontWeight: 400, letterSpacing: '0.28em', textTransform: 'uppercase', opacity: 0.65, marginBottom: 6, color: 'var(--navy)' }}>{line1}</div>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 26, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--navy)' }}>{line2 || 'Reconciliador'}</div>
          </div>
        </div>
        <div style={{ textAlign: 'center', marginTop: 56, marginBottom: 40 }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 80, fontWeight: 700, lineHeight: 1.1, color: 'var(--gold-escuro)' }}>{title}</div>
          {date && <div style={{ marginTop: 18, fontFamily: 'var(--font-wide)', fontSize: 22, fontWeight: 600, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--gold-escuro)' }}>{date}</div>}
        </div>
        {passages.filter(Boolean).length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, paddingBlock: 36, borderTop: '1px solid var(--linha)', borderBottom: '1px solid var(--linha)', marginBottom: 48 }}>
            {passages.filter(Boolean).map((p, i) => <div key={i} style={{ fontFamily: 'var(--font-display)', fontWeight: 700, color: 'var(--navy)', lineHeight: 1.15, fontSize: 46 }}>{p}</div>)}
          </div>
        )}
        <div style={{ flex: 1, fontFamily: 'var(--font-sans)', lineHeight: 1.5, color: 'var(--grafite)', textAlign: 'left', fontSize: 40 }} dangerouslySetInnerHTML={{ __html: body || '' }} />
        <div style={{ marginTop: 32, paddingTop: 24, fontFamily: 'var(--font-wide)', fontSize: 24, fontWeight: 600, letterSpacing: '0.1em', color: 'var(--navy)', textAlign: 'center' }}>{getBrandHandle()}</div>
        <WaveFooter color="var(--gold)" opacity={0.08} />
      </div>
    );
  }
    ...
    return (
      <div className="t-story">
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <div style={{ fontFamily: 'var(--font-script)', fontSize: 72, lineHeight: 1.3, color: 'var(--navy)', textWrap: 'balance' }}>{verse}</div>
          <div style={{ marginTop: 48, fontFamily: 'var(--font-wide)', fontSize: 28, fontWeight: 600, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--gold-escuro)' }}>{reference}</div>
        </div>
        <div style={{ fontFamily: 'var(--font-wide)', fontSize: 24, fontWeight: 500, letterSpacing: '0.1em', color: 'var(--grafite-3)' }}>{getBrandHandle()}</div>
    ...
    return (
      <div className="t-story t-story--photo">
        ...
        <div style={{ position: 'relative', zIndex: 2, width: '100%', height: '100%', padding: '120px 96px', display: 'flex', flexDirection: 'column', color: 'var(--papel)' }}>
          ...
          <div style={{ flex: 1 }} />
          <div style={{ fontFamily: 'var(--font-wide)', fontSize: 28, fontWeight: 600, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--gold-claro)', marginBottom: 32 }}>{kicker}</div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 108, lineHeight: 1.1, fontWeight: 700, color: 'var(--papel)', marginBottom: 56, textWrap: 'balance' }}>{title}</div>
          <div style={{ fontFamily: 'var(--font-sans)', fontSize: 36, fontWeight: 500, color: 'var(--papel)', lineHeight: 1.4 }}>
          ...
          <div style={{ height: 96 }} />
          <div style={{ display: 'inline-flex', alignSelf: 'flex-start', alignItems: 'center', gap: 16, padding: '20px 32px', background: 'var(--gold)', color: 'var(--navy)', fontFamily: 'var(--font-wide)', fontSize: 26, fontWeight: 700, letterSpacing: '0.06em', borderRadius: 999 }}>
    ...
    return (
      <div className="t-story">
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 48 }}>
          ...
          <div style={{ fontFamily: 'var(--font-script)', fontSize: 60, lineHeight: 1.3, color: 'var(--navy)', textWrap: 'balance' }}>"{quote}"</div>
          <div style={{ fontFamily: 'var(--font-wide)', fontSize: 24, fontWeight: 600, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--gold-escuro)' }}>{who}</div>
        </div>
        <div style={{ fontFamily: 'var(--font-wide)', fontSize: 22, fontWeight: 500, letterSpacing: '0.1em', color: 'var(--grafite-3)' }}>{getBrandHandle()}</div>
      </div>
    );
  }
    ...
    return (
      <div className="t-print">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ fontFamily: 'var(--font-wide)', fontSize: 18, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--grafite-3)' }}>{date}</div>
        </div>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 32 }}>
          <div style={{ fontFamily: 'var(--font-wide)', fontSize: 18, fontWeight: 700, letterSpacing: '0.24em', textTransform: 'uppercase', color: 'var(--gold-escuro)' }}>Boletim Litúrgico</div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 120, lineHeight: 1, fontWeight: 700, color: 'var(--navy)', textWrap: 'balance' }}>{title}</div>
          <div style={{ fontFamily: 'var(--font-script)', fontSize: 34, color: 'var(--gold-escuro)', maxWidth: '30ch', textWrap: 'pretty' }}>{subtitle}</div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', paddingTop: 24, borderTop: '1px solid var(--linha)', fontFamily: 'var(--font-sans)', fontSize: 18, color: 'var(--grafite-2)' }}>
          <div>
